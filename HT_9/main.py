...
def check_balance(user):
    with open(f'user_balance.json', 'r+') as my_file:
...
def append_balance(user):
    with open(f'user_balance.json', 'r+') as my_file1:
    ...
    while flag == True:
        try:
            ...
            with open(f'user_balance.json', 'w') as f:
...
cur.execute('''CREATE TABLE IF NOT EXISTS inkasator_check
               (banknotes text, quantity interger)''')
cur.execute('''CREATE TABLE IF NOT EXISTS user_balance
               (username text, balance interger)''')
...
for user in user_list:
    exists = cur.execute('''SELECT username, balance FROM user_balance WHERE username=?''', (user[0],)).fetchone()
    if not exists:
        cur.execute('''INSERT INTO user_balance VALUES (?, ?) ''', user)
...
with con:
    ...
    for row in rows:
        user_balance_list.append(row)

for i in banknotes_list:
    banknotes_dict[i[0]] = i[1]

with open('money_data.json', 'w') as f2:
    json.dump(banknotes_dict, f2, indent = 2)


for i in user_balance_list:
    user_balance_dict[i[0]] = i[1]

with open('user_balance.json', 'w') as f2:
    json.dump(user_balance_dict, f2, indent = 2)


result = check_usernames()
for something in same_list:
    if something[0] == result and something[2] == 0:
        main_func(result)
    elif something[0] == result and something[2] == 1:
        ink_main_func(result)


with open('money_data.json', 'r+') as f1:
    banknotes_result_dict = json.load(f1)

banknotes_result_list = []
for k, v in banknotes_result_dict.items():
    t = k, v
    banknotes_result_list.append(t)

cur.execute('DELETE FROM inkasator_check;',);

with open('user_balance.json', 'r+') as f1:
    user_balance_result_dict = json.load(f1)

user_balance_result_list = []
for k, v in user_balance_result_dict.items():
    t = k, v
...
for user in banknotes_result_list:
    exists = cur.execute('''SELECT banknotes, quantity FROM inkasator_check WHERE banknotes=?''', (user[0],)).fetchone()
    if not exists:
        cur.execute('''INSERT INTO inkasator_check VALUES (?, ?) ''', user)
